...
export const ServiceButton = ({ icon: Icon, label, onClick }: ServiceButtonProps) => {
  return (
    <Button
      ...
      variant="outline"
    >
      <Icon strokeWidth={1.5} className="w-24 h-24 text-ayurveda-primary" />
      <span className="text-base font-medium text-center font-sans whitespace-pre-line leading-tight">{label}</span>
    </Button>
  );
};